...
const FeaturePost = ({post}) => {
  ...
  return (
    <div className='featurePost'>
        ...
        {post.photo && (<img src={PF + post.photo} className="img-fluid featureImg" alt=""/>)}
        <div className="col-md-12 px-0 mb-5">
          ...
          <p className="my-3 fs-3">{post.desc}</p>
          <Link to={`/posts/${post._id}`} className="fs-4">Continue reading..</Link>
        </div>
        <p>&nbsp;</p>
    </div>
  )
